...
impl<'r> FromRequest<'r> for AuthUser {
	...
	async fn from_request(req: &'r Request<'_>) -> RequestOutcome<Self, Self::Error> {
		...
		if let Some(bearer) = auth.and_then(|h| h.strip_prefix("Bearer ")) {
			...
			match decode::<Claims>(bearer, &DecodingKey::from_secret(secret.as_bytes()), &validation) {
				Ok(data) => {
					let role = match data.claims.role.as_str() { "admin" | "Admin" => Role::Admin, _ => Role::Reader };
					return Outcome::Success(AuthUser { subject: data.claims.sub, role });
				}
				Err(_) => return Outcome::Error((Status::Unauthorized, AppError::Unauthorized)),
			}
		}
		Outcome::Success(AuthUser { subject: "public".into(), role: Role::Reader })
	}
}

impl AuthUser {
	pub fn require_admin(&self) -> AppResult<()> {
		if self.role != Role::Admin { return Err(AppError::Forbidden); }
		Ok(())
	}
}

// Optional ed25519 body signature verification
pub fn verify_detached_signature(_body: &[u8], _signature_b64: Option<&str>, _pubkey_b58: Option<&str>) -> AppResult<()> {
